...
import { simulateMortgagePlan } from "../../services/mortgages/simulateService.js";
import { periodKeyToMonthStart } from "../../services/mortgages/scheduleMath.js";
...
router.post("/:id/simulate", async (req, res) => {
  try {
    ...
    const months = Math.min(600, Math.max(1, parseInt(String(req.body?.months || "360"), 10) || 360));
    ...
    const toKey = dayjs(periodKeyToMonthStart(from)).add(months - 1, "month").format("YYYY-MM");
    ...
    const paymentsByPk = new Map();
    for (const p of payments) {
      const pk = String(p.periodKey);
      if (!paymentsByPk.has(pk)) paymentsByPk.set(pk, []);
      paymentsByPk.get(pk).push(p);
    }
    ...
    res.json({
      ...
      mortgage: { title: exp.title, mortgageHolder: exp.mortgageHolder, mortgageKind: exp.mortgageKind },
      scenario,
      ...sim,
    });
  } catch (err) {
