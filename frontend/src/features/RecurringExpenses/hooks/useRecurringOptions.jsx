// src/features/RecurringExpenses/hooks/useRecurringOptions.js
...
export const useRecurringOptions = ({ expenses, filter, now = new Date() }) => {
  ...
  const forecast = useMemo(() => {
    return months.map((m) => {
      ...
      const items = filtered.map((e) => {
        ...
        const maxDay = e.type === "MORTGAGE" ? 31 : 28;
        const dueInMonth = new Date(m.getFullYear(), m.getMonth(), Math.min(due.getDate(), maxDay));
  ...
  const selectedMonth = useCallback(
    (selectedMonthKey) => forecast.find((m) => m.key === selectedMonthKey) ?? null,
    [forecast]
  );
